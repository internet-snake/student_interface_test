...
@pytest.mark.usefixtures("answerread")
@allure.epic("学生考试系统答卷查询测试")
class TestAnswerRead:

    @pytest.mark.parametrize("caseinfo", read_yaml("./data/test_answer_read.yaml"))
    def test_answer_read(self, caseinfo):
        ...
        assert login_res.json().get("code") == 1, "登录前置条件失败，无法继续答卷查询测试"
        logger.info("前置登录成功")
