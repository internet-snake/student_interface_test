...
@allure.epic("学生考试系统学科列表测试")
class TestSubjectList:

    @pytest.mark.parametrize("caseinfo", read_yaml("./data/test_subject_list.yaml"))
    def test_subject_list(self, caseinfo):
        ...
        assert login_res.json().get("code") == 1, "登录前置条件失败，无法继续学科列表测试"
        logger.info("前置登录成功")
